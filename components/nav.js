...
export default function Nav() {
    return (
        <nav className="flex fixed items-center bg-transparent md:p-3 flex-wrap justify-center h-auto absolute w-full">
            ...
            >
                <ul className="md:inline-flex flex-row md:ml-auto md:w-auto w-full md:items-center items-start flex md:h-auto md:space-x-6">
                    {links.map(({ href, label }) => (
                        <li
                            key={`${href}${label}`}
                            className="flex inline-flex md:inline-flex md:w-auto w-full py-2 rounded text-dark items-center justify-center"
                        >
